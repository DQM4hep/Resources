...
A new generic framework for data quality monitoring for high energy phisics, DQM4hep, has been developed without any assumption on the underlying event data model implementation or data format. It provides tools to develop and deploy an experiment monitoring system during data taking or for monitoring offline data, either from simulation or from recorded data from detector on disk. Visualization interfaces have been developed with Qt4~\cite{QT} libraries for steering the online deployment and displaying the monitor elements collected during data taking.

Different detector prototypes have been using this framework for several test beam campaigns to monitor their data taking. For some of these campaigns, the data acquisition was performed with multiple sub-detectors, demonstrating the scalability of the framework.
